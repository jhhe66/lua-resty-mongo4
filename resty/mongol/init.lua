    ...
    elseif r.ismaster then return self 
        ...
        for i , v in ipairs ( r.hosts ) do
            if not searched[v] then
                ...
                local conn = new()
                local ok, err = conn:connect(host, port, self.options_table)
                if not ok then
...
function connmethods:connect(host, port, options_table)
    self.host = host or self.host
    self.port = port or self.port
    self.options_table = options_table or self.options_table
    local sock = self.sock

    if nil == options_table then
        return sock:connect(self.host, self.port)
    else
        return sock:connect(self.host, self.port, self.options_table)
    end
end
